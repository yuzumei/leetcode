class Solution:
    def assignTasks(self, servers, tasks):
        ...
        queue = sorted(list(range(n)),key= lambda x:servers[x])
        time = 0
        ans = []
        wait = list(range(n))
        import collections
        '''记录每个时间点结束运行的服务器'''
        time_memo = collections.defaultdict(list)
        while len(ans) < m:
            if time in time_memo:
                use = time_memo[time]
                for item in use:
                    bisect.insort(wait,item)
                del time_memo[time]
            if not wait:
                time = min(time_memo)
                continue
            ...
            task = tasks.pop(0)
            enter = wait.pop(0)
            time_memo[time+task].append(enter)
            time += 1
            ans.append(queue[enter])
        return ans[:m]

x=Solution()
print(x.assignTasks([338,890,301,532,284,930,426,616,919,267,571,140,716,859,980,469,628,490,195,664,925,652,503,301,917,563,82,947,910,451,366,190,253,516,503,721,889,964,506,914,986,718,520,328,341,765,922,139,911,578,86,435,824,321,942,215,147,985,619,865]
